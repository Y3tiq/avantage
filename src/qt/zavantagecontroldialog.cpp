...
#include "ui_zavantagecontroldialog.h"
...
std::list<std::string> ZAvantageControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZAvantageControlDialog::listMints;

ZAvantageControlDialog::ZAvantageControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZAvantageControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZAvantageControlDialog::~ZAvantageControlDialog()
{
    delete ui;
}

void ZAvantageControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZAvantageControlDialog::updateList()
{
...
void ZAvantageControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZAvantageControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZAvantage_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZAvantageControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZAvantageControlDialog::GetSelectedMints()
{
...
void ZAvantageControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
